...
### Theorem: Dimension of Sum of Subspaces
...
Since \( B \) is a linearly independent subset of both \( W_1 \) and \( W_2 \), by the **Basis Extension Theorem**, we can extend it to form bases for both subspaces:

1. **For \( W_1 \)**: Extend \( B \) with \( \{ \s_1, \dots, \s_p \} \) such that \( B_1 = \{ \b_1, \dots, \b_n, \s_1, \dots, \s_p \} \) is a basis of \( W_1 \).
   - Note: \( \dim(W_1) = n + p \).

2. **For \( W_2 \)**: Extend \( B \) with \( \{ \t_1, \dots, \t_q \} \) such that \( B_2 = \{ \b_1, \dots, \b_n, \t_1, \dots, \t_q \} \) is a basis of \( W_2 \).
   - Note: \( \dim(W_2) = n + q \).
...
Observe the vector \( \v \):

1. From the LHS, \( \v \in W_1 \).
2. From the RHS, \( \v \in W_2 \).
3. Therefore, \( \v \in W_1 \cap W_2 \).
