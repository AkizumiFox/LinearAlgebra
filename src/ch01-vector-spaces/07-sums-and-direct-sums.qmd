...
Next, we show that \( W_1 \cap W_2 = \{ \O \} \). Let \( \A \in W_1 \cap W_2 \). That implies that \( \A \in W_1 \) and \( \A \in W_2 \). That means we have the identites \( \A = \A^{\top} \) and \( \A = - \A^{\top} \). Therefore, \( \A = - \A \), which implies that \( \A = \O \).
:::
